...
def __init_process(*args):
    ...
    epoch, vocab, syn0_tmp, syn1_tmp, table, cbow, neg, dim, starting_alpha, win, num_processes, global_word_count = args[:-1]
    fi = open(args[-1], 'r')
...
def train(fi, fo, cbow, neg, dim, alpha, win, min_count, num_processes, binary, epoch):
    ...
    pool = Pool(processes=num_processes, initializer=__init_process,
                initargs=(epoch, vocab, syn0, syn1, table, cbow, neg, dim, alpha,
...
if __name__ == '__main__':
    ...
    parser.add_argument('-lr', help='Starting lr', dest='lr', default=0.025, type=float)
    ...
    parser.add_argument('-sample', help='Number of training epochs', dest='sample', default=1, type=float)
    args = parser.parse_args()

    train(args.fi, args.fo, bool(args.cbow), args.neg, args.dim, args.lr, args.win,
