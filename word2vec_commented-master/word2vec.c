...
real_t *syn0, *syn1, *syn1neg, *expTable;
clock_t start;
    ...
    while (!feof(fin))
    {
        ...
        if ((ch == ' ') || (ch == '\t') || (ch == '\n'))
        {
            ...
            if (a > 0)
            {
                ...
                // (as the end-of-sentence token, i.e. '</s>' here.
                if (ch == '\n')
                    ungetc(ch, fin);
                break;
            }
...
int AddWordToVocab(char *word)
{
    ...
    if (vocab_size + 2 >= vocab_max_size) {
        vocab_max_size += 1000;
        vocab = (struct vocab_word *)real_tloc(vocab, vocab_max_size * sizeof(struct vocab_word));
    }
...
void SortVocab()
{
    ...
    vocab = (struct vocab_word *)real_tloc(vocab, (vocab_size + 1) * sizeof(struct vocab_word));
...
void InitNet()
{
    ...
    a = posix_memalign((void **)&syn0, 128, (long long)vocab_size * layer1_size * sizeof(real_t));
    ...
    if (hs)
    {
        a = posix_memalign((void **)&syn1, 128, (long long)vocab_size * layer1_size * sizeof(real_t));
    ...
    if (negative > 0)
    {
        ...
        a = posix_memalign((void **)&syn1neg, 128, (long long)vocab_size * layer1_size * sizeof(real_t));
    ...
    for (a = 0; a < vocab_size; a++)
        for (b = 0; b < layer1_size; b++)
        {
            ...
            syn0[a * layer1_size + b] = (((next_random & 0xFFFF) / (real_t)65536) - 0.5) / layer1_size;
        }
...
void *TrainModelThread(void *id)
{
    ...
    real_t *neu1 = (real_t *)calloc(layer1_size, sizeof(real_t));

    // neu1e is used by both architectures.
    real_t *neu1e = (real_t *)calloc(layer1_size, sizeof(real_t));
        ...
        if (word_count - last_word_count > 10000)
        {
            ...
            if ((debug_mode > 1))
            {
                ...
                printf("%cAlpha: %f  Progress: %.2f%%  Words/thread/sec: %.2fk  ", 13, alpha,
                    word_count_actual / (real_t)(iter * train_words + 1) * 100,
                    word_count_actual / ((real_t)(now - start + 1) / (real_t)CLOCKS_PER_SEC * 1000));
                fflush(stdout);
            }
            ...
            while (1)
            {
                ...
                */
                if (sample > 0)
                {
                    ...
                    real_t ran = (sqrt(vocab[word].cn / (sample * train_words)) + 1) * (sample * train_words) / vocab[word].cn;
                    ...
                    if (ran < (next_random & 0xFFFF) / (real_t)65536) continue;
                }
...
void TrainModel()
{
    ...
    if (classes == 0)
    {
        ...
        for (a = 0; a < vocab_size; a++)
        {
            ...
            if (binary) for (b = 0; b < layer1_size; b++) fwrite(&syn0[a * layer1_size + b], sizeof(real_t), 1, fo);
    ...
    else
    {
        ...
        real_t closev, x;
        real_t *cent = (real_t *)calloc(classes * layer1_size, sizeof(real_t));
